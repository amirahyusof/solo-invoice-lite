    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/invoices')} className="p-2 bg-white border border-mate-cream rounded-lg text-mate-forest hover:bg-mate-cream transition-colors">
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <div className="flex flex-wrap gap-2">
          {invoice.status !== 'paid' && (
            <Link to={`/invoices/edit/${id}`} className="px-4 py-2 bg-white border border-mate-cream rounded-lg font-bold text-mate-dark flex items-center gap-2 hover:bg-mate-cream transition-colors">
              <Edit size={18} /> Edit
            </Link>
          )}
          ...
          {invoice.status !== 'paid' && (
            <button onClick={() => setIsReceiptModalOpen(true)} className="px-4 py-2 bg-green-600 text-white rounded-lg font-bold flex items-center gap-2 hover:bg-green-700 transition-colors">
              <CheckCircle size={18} /> Mark as Paid
            </button>
          )}
          {receipt && (
            <Link to={`/receipts/${receipt.id}`} className="px-4 py-2 bg-mate-brown text-white rounded-lg font-bold flex items-center gap-2 hover:bg-mate-brown/90 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-8 md:p-12 flex-1">
              <div className="flex flex-col md:flex-row justify-between mb-12">
                <div className="flex items-start gap-4">
                  {settings.logo && (
                    <img src={settings.logo} alt="Business Logo" className="w-20 h-20 object-contain rounded-lg border border-mate-cream bg-white p-2" />
                  )}
                  <div>
                    <h1 className="text-3xl font-black text-mate-forest mb-2">{settings.businessName}</h1>
                    <p className="text-mate-forest/60 text-sm whitespace-pre-line">{settings.address}</p>
                  </div>
                </div>
                <div className="text-left md:text-right mt-6 md:mt-0">
                  <h2 className="text-4xl font-bold text-mate-dark opacity-20 uppercase tracking-widest mb-4">Invoice</h2>
                  <p className="font-bold text-mate-forest">{invoice.invoiceNo}</p>
                  <p className="text-sm text-mate-forest/60">Date: {formatDate(invoice.issueDate)}</p>
                  <p className="text-sm text-mate-forest/60 font-bold">Due: {formatDate(invoice.dueDate)}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 border-y border-mate-cream py-8">
                <div>
                  <h3 className="text-xs font-bold uppercase text-mate-brown mb-2 tracking-wider">Bill To:</h3>
                  <p className="font-bold text-lg text-mate-dark">{client.name}</p>
                  {client.company && <p className="font-medium text-mate-forest">{client.company}</p>}
                  <p className="text-sm text-mate-forest/70 whitespace-pre-line">{client.address}</p>
                </div>
                <div>
                  <h3 className="text-xs font-bold uppercase text-mate-brown mb-2 tracking-wider">Payment Info:</h3>
                  <p className="text-sm text-mate-forest font-medium">{settings.bankName}</p>
                  <p className="text-sm text-mate-dark font-bold">Acc No: {settings.bankAccountNo}</p>
                </div>
              </div>

              <table className="w-full mb-12">
                <thead className="border-b-2 border-mate-dark">
                  <tr>
                ...
                <tbody className="divide-y divide-mate-cream/50">
                  {items.map((item, idx) => (
                    <tr key={idx}>
                      <td className="py-4 text-mate-dark font-medium">{item.description}</td>
              ...
              <div className="flex justify-end">
                <div className="w-full md:w-64 space-y-3">
                  <div className="flex justify-between font-black text-2xl pt-3 border-t-2 border-mate-forest text-mate-dark">
                    <span>Total</span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-8 rounded-2xl border border-mate-cream shadow-sm flex flex-col items-center">
            <h3 className="font-black text-mate-dark mb-6 flex items-center justify-center gap-2 uppercase tracking-widest text-sm text-center">
              <Share2 size={20} className="text-mate-brown" /> Quick Share
            </h3>
          ...
          <div className="bg-white p-6 rounded-xl border border-mate-cream shadow-sm">
            <h3 className="font-black text-xs uppercase text-mate-brown mb-3 tracking-widest">Client Contact</h3>
      ...
      {isReceiptModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-mate-cream flex justify-between items-center bg-green-600 text-white">
            ...
            <form onSubmit={handleMarkPaid} className="p-6 space-y-4">
              <div>
                ...
                <input required name="paidDate" type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full px-4 py-3 rounded-xl border-2 border-mate-forest/20 bg-white focus:ring-4 focus:ring-mate-brown/20 outline-none font-bold" />
              </div>
              <div>
                <label className="block text-sm font-bold text-mate-dark mb-1">Method</label>
                <select name="paymentMethod" className="w-full px-4 py-3 rounded-xl border-2 border-mate-forest/20 bg-white focus:ring-4 focus:ring-mate-brown/20 outline-none font-bold">
                  <option value="Bank Transfer">Bank Transfer</option>
                  <option value="Cash">Cash</option>
                </select>
              </div>
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={() => setIsReceiptModalOpen(false)} className="flex-1 px-4 py-2 border border-mate-cream rounded-lg font-bold">Cancel</button>
